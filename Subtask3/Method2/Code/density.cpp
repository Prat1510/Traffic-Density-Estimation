...
void makeBackground(string name, int X, int Y){
    ...
    imwrite("../Analysis/background_" + to_string(X) + "_" + to_string(Y) + ".jpg" , frame);
}
...
int processVideo(string name, int X,int Y )
{
    ...
    Mat frame = imread("../Analysis/background_" + to_string(X) + "_" + to_string(Y) + ".jpg");       //reading empty image of road for using as background
    ...
    Mat fgMask1;
    ofstream myfile;
    myfile.open("../Analysis/output_" + to_string(X) + "_" + to_string(Y) + ".txt");
